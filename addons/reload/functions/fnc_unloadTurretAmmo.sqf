...
private ["_cannon","_turrets","_actions","_magazinesAllTurrets"];
...

{
	private ["_weapons", "_weapon", "_maxMagazines", "_magazines", "_magazine"];
		...
		for "_i" from 0 to ((count _weapons) - 1) do {
			...
			_magazines = _vehicle magazinesTurret _x;
			if ((_weapon != "") && (count _magazines > 0)) then {
			
			    if([ _weapon ,_vehicle] call FUNC(isGunTurret)) then {                      

				    for "_j" from 0 to ((count _magazines) -1) do {
					    _magazine = _magazines select _j;						
						if(_magazine in (getArray (configFile >> "CfgWeapons" >> _weapon >> "Magazines"))) then {
						    ...
						    _count = [_vehicle,_weapon,_magazine,_x] call FUNC(countTurretMag);
						        ...
						        [
						            ...
						            _mag_icon, // ICON
							        {
								        ...
								        [
									        _this select 0,
									        _this,
									       { (_this select 0) call FUNC(startUnload) },
									       {},
									       localize LSTRING(unloadingCannon)
								        ] call FUNC(progressBar);
								        //(_this select 0) spawn FUNC(EndAnim);
							        }, // STATEMENT
							        {true}, // CONDITION
							        {},
							        [_reloadTime,_vehicle,_magazine,_x,_count] // ARGUMENTS
